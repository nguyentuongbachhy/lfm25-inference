...
extern "C" __global__
__launch_bounds__(ATTENTION_FLASH_BLOCK_SIZE)
void segmented_prefill_gqa_lfm2_bf16_flash(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key,
    const __nv_bfloat16* __restrict__ value,
    const uint32_t* __restrict__ segment_offsets,
    ...
    const uint32_t seg_idx = blockIdx.y;
    if (seg_idx >= num_segments) {
        return;
    }
    const size_t seg_start = segment_offsets[seg_idx];
    const size_t seg_end = segment_offsets[seg_idx + 1];
    if (seg_end <= seg_start || seg_start >= total_tokens) {
        return;
    }
    const size_t seq_len = seg_end - seg_start;
    const size_t q_tile_in_seg = blockIdx.x / LFM2_NUM_KV_HEADS;
    const uint32_t kv_head = blockIdx.x % LFM2_NUM_KV_HEADS;
    if (q_tile_in_seg * TILE_Q >= seq_len) {
        return;
    }
    const size_t q_start = seg_start + q_tile_in_seg * TILE_Q;

    const uint32_t lane = threadIdx.x & 31U;
    const uint32_t warp = threadIdx.x >> 5U; // 0..3 (4 warps = 128 threads)

    __shared__ __align__(16) __nv_bfloat16 s_Q[4][TILE_Q * LFM2_HEAD_DIM];
    __shared__ __align__(16) __nv_bfloat16 s_K[TILE_K * LFM2_HEAD_DIM];
    __shared__ __align__(16) __nv_bfloat16 s_V[TILE_K * LFM2_HEAD_DIM];

    __shared__ __align__(16) float s_S[4][TILE_Q * TILE_K];
    __shared__ __align__(16) __nv_bfloat16 s_P[4][TILE_Q * TILE_K];
    __shared__ __align__(16) float s_O_tile[4][TILE_Q * 16];
    ...
    for (size_t elem = threadIdx.x; elem < 4 * TILE_Q * LFM2_HEAD_DIM; elem += blockDim.x) {
        ...
        const size_t token = q_start + t;
        const uint32_t q_head = kv_head * LFM2_Q_PER_KV + h;
        if (token < seg_end && token < total_tokens) {
            const size_t src = (token * LFM2_NUM_Q_HEADS + q_head) * LFM2_HEAD_DIM + d;
            s_Q[h][t * LFM2_HEAD_DIM + d] = query[src];
        } else {
            s_Q[h][t * LFM2_HEAD_DIM + d] = __float2bfloat16_rn(0.0f);
    ...
    const size_t q_token_max = (q_start + TILE_Q < seg_end) ? (q_start + TILE_Q) : seg_end;
    const size_t k_tiles_end = (q_token_max - seg_start + TILE_K - 1) / TILE_K;

    for (size_t k_tile = 0; k_tile < k_tiles_end; ++k_tile) {
        const size_t k_start = seg_start + k_tile * TILE_K;
        ...
        for (int j = 0; j < 16; ++j) {
            const size_t k_token = k_start + j;
            const size_t q_token = q_start + r;
            if (k_token >= seg_start && k_token <= q_token && k_token < seg_end && q_token < seg_end) {
                const float score = s_S[warp][r * 16 + j] * LFM2_ATTN_SCALE;
                if (score > m_curr) m_curr = score;
            }
        }
        ...
        for (int j = 0; j < 16; ++j) {
            ...
            const size_t q_token = q_start + r;
            float p = 0.0f;
            if (k_token >= seg_start && k_token <= q_token && k_token < seg_end && q_token < seg_end && m_curr != -INFINITY) {
                p = __expf(s_S[warp][r * 16 + j] * LFM2_ATTN_SCALE - m_new);
            }
            if (d_half == 0) {
                s_P[warp][r * 16 + j] = __float2bfloat16_rn(p);
    ...
    if (q_start + r < seg_end && q_start + r < total_tokens) {
        const size_t q_token = q_start + r;
        const uint32_t q_head = kv_head * LFM2_Q_PER_KV + warp;
        const size_t out_base = (q_token * LFM2_NUM_Q_HEADS + q_head) * LFM2_HEAD_DIM + d_base;
        const float inv_L = (L_run > 0.0f) ? (1.0f / L_run) : 0.0f;
