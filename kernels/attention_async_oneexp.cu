...
constexpr int ATTENTION_ASYNC_ONEEXP_BLOCK_SIZE = 256;
...
__device__ __forceinline__ void paged_gqa_lfm2_bf16_async_oneexp_body(
    ...
    for (size_t page = 0; page <= last_page; ++page) {
        ...
        if (active) {
            ...
            for (size_t key_offset = 0; key_offset < page_tokens; ++key_offset) {
                ...
                // Stable online softmax with one exponential per key token.
                // If the new score is below the running maximum, only the new
                // contribution needs scaling. If it raises the maximum, only
                // the accumulated state needs rescaling.
                if (dot <= maximum) {
                    const float weight = expf(dot - maximum);
                    accumulator0 = fmaf(value0, weight, accumulator0);
                    accumulator1 = fmaf(value1, weight, accumulator1);
                    denominator += weight;
                } else {
                    const float old_scale = expf(maximum - dot);
                    accumulator0 = fmaf(accumulator0, old_scale, value0);
                    accumulator1 = fmaf(accumulator1, old_scale, value1);
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_ONEEXP_BLOCK_SIZE)
void paged_gqa_lfm2_bf16_async_oneexp_ps16(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_table,
    ...
    paged_gqa_lfm2_bf16_async_oneexp_body<16, false>(
        query, key_cache, value_cache, block_table, nullptr, position_ids,
        output, num_tokens, num_pages, block_table_length, 1
    );
}

extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_ONEEXP_BLOCK_SIZE)
void paged_gqa_lfm2_bf16_async_oneexp_ps32(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_table,
    ...
    paged_gqa_lfm2_bf16_async_oneexp_body<32, false>(
        query, key_cache, value_cache, block_table, nullptr, position_ids,
        output, num_tokens, num_pages, block_table_length, 1
    );
}
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_ONEEXP_BLOCK_SIZE)
void paged_ragged_gqa_lfm2_bf16_async_oneexp_ps16(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_tables,
    ...
    paged_gqa_lfm2_bf16_async_oneexp_body<16, true>(
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_ONEEXP_BLOCK_SIZE)
void paged_ragged_gqa_lfm2_bf16_async_oneexp_ps32(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_tables,
    ...
    paged_gqa_lfm2_bf16_async_oneexp_body<32, true>(
        query, key_cache, value_cache, block_tables, request_slots, position_ids,
        output, num_tokens, num_pages, block_table_stride, block_table_rows
    );
}
