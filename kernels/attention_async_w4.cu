...
constexpr int ATTENTION_ASYNC_W4_BLOCK_SIZE = 128;
...
__device__ __forceinline__ void paged_gqa_lfm2_bf16_async_w4_body(
    ...
    const float q0 = __bfloat162float(query[query_base + dim0]);
    const float q1 = __bfloat162float(query[query_base + dim1]);
    ...
    for (size_t page = 0; page <= last_page; ++page) {
        ...
        for (size_t key_offset = 0; key_offset < page_tokens; ++key_offset) {
            ...
            // Algebraically equivalent online softmax with one expf per token.
            // dot is warp-uniform after the broadcast, so this branch does not
            // introduce intra-warp divergence.
            if (dot <= maximum) {
                const float scale = expf(dot - maximum);
                accumulator0 += value0 * scale;
                accumulator1 += value1 * scale;
                denominator += scale;
            } else {
                const float scale = expf(maximum - dot);
                accumulator0 = accumulator0 * scale + value0;
                accumulator1 = accumulator1 * scale + value1;
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_W4_BLOCK_SIZE)
void paged_gqa_lfm2_bf16_async_w4_ps16(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_table,
    ...
    paged_gqa_lfm2_bf16_async_w4_body<16, false>(
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_W4_BLOCK_SIZE)
void paged_gqa_lfm2_bf16_async_w4_ps32(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_table,
    ...
    paged_gqa_lfm2_bf16_async_w4_body<32, false>(
        query, key_cache, value_cache, block_table, nullptr, position_ids,
        output, num_tokens, num_pages, block_table_length, 1
    );
}
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_W4_BLOCK_SIZE)
void paged_ragged_gqa_lfm2_bf16_async_w4_ps16(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_tables,
    ...
    paged_gqa_lfm2_bf16_async_w4_body<16, true>(
...
extern "C" __global__
__launch_bounds__(ATTENTION_ASYNC_W4_BLOCK_SIZE)
void paged_ragged_gqa_lfm2_bf16_async_w4_ps32(
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key_cache,
    const __nv_bfloat16* __restrict__ value_cache,
    const uint32_t* __restrict__ block_tables,
    ...
    paged_gqa_lfm2_bf16_async_w4_body<32, true>(
        query, key_cache, value_cache, block_tables, request_slots, position_ids,
        output, num_tokens, num_pages, block_table_stride, block_table_rows
    );
}
