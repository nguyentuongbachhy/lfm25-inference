...
pub fn rope_qk_bf16_inplace(
    ...
    let head_dim = query_head_dim;
        ...
        "RoPE head_dim={head_dim} exceeds \
         supported maximum {MAX_HEAD_DIM}",
    );
    ...
    let key_prefix = &key_dims[..key_rank - 2];
        ...
        "RoPE Q/K token dimensions mismatch: \
    ...
    let q_values_per_token = num_q_heads
        ...
        query.numel() % q_values_per_token == 0,
    ...
    let half_dim = head_dim / 2;
        ...
        "RoPE position_ids mismatch: \
         expected {num_tokens} positions, got {:?}",
        position_ids.dims(),
    );
