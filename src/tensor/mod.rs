...
impl<T> BufferPool<T> {
    ...
    fn owner_state_mut(&self) -> &mut BufferPoolState<T> {
        ...
        // SAFETY: after enter_owner_mode the pool is exclusively accessed by
        // the GPU-owner thread. The debug assertion catches invariant breaks in
        // development without synchronization in release hot paths.
        unsafe {
...
mod tests {
    ...
    #[test]
    fn pool_owner_mode_is_one_way() {
        let pool = BufferPool::<u8>::new(1024);
        pool.enter_owner_mode().expect("first transition");
        assert!(pool.enter_owner_mode().is_err());
        assert_eq!(pool.stats().available_elements, 0);
    }
}
