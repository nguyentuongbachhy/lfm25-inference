...
fn mxfp8_block32_dynamic_scales_match_bf16_small_shape() -> Result<()> {
    ...
    let unit_host = vec![bf16::from_f32(1.0); MXFP8_K_BLOCK];
    let unit = runtime.upload(&unit_host, Shape::new([1, MXFP8_K_BLOCK]))?;
    let mut unit_fp8 = runtime.zeros::<u8>(Shape::new([1, MXFP8_K_BLOCK]))?;
    let mut unit_scales =
        runtime.zeros::<u8>(Shape::new([scale_storage_len(1, MXFP8_K_BLOCK)?]))?;
    unsafe {
        quantizer.launch(
            runtime.stream(),
            unit.storage(),
            unit_fp8.storage_mut(),
            unit_scales.storage_mut(),
            1,
            MXFP8_K_BLOCK,
        )?;
    }
    runtime.synchronize()?;
    let unit_scales_host = readback(&runtime, &unit_scales)?;
    ensure!(
        unit_scales_host[0] == 119,
        "MXFP8 1.0 block expected E8M0 scale code 119, got {}",
        unit_scales_host[0]
    );
    ensure!(
        unit_scales_host[1..].iter().all(|value| *value == 0),
        "MXFP8 padded scale storage was modified"
    );
    ...
    let candidate = readback(&runtime, &candidate)?;
        ...
        candidate.iter().all(|value| value.to_f32().is_finite()),
...
#[test]
#[ignore = "GPU research benchmark for BF16 vs tensorwide FP8 vs MXFP8 block32"]
fn bench_mxfp8_block32_outlier_shapes() -> Result<()> {
    ...
    for (site, n, k) in [
        ...
        for m in [1usize, 2, 8] {
            ...
            let mxfp8_e2e = benchmark_gpu_paired(
                runtime.context(),
                runtime.stream(),
                bench,
                || unsafe {
            ...
            println!(
                "mxfp8_outlier site={site} M={m} N={n} K={k} \
                 tensorwide_quant_mean_us={:.3} tensorwide_gemm_mean_us={:.3} \
                 tensorwide_bf16_mean_us={:.3} tensorwide_e2e_mean_us={:.3} \
                 tensorwide_e2e_p95_us={:.3} tensorwide_speedup_vs_bf16={:.4}x \
                 mxfp8_quant_mean_us={:.3} mxfp8_gemm_mean_us={:.3} \
                 mxfp8_bf16_mean_us={:.3} mxfp8_e2e_mean_us={:.3} \
                 mxfp8_e2e_p95_us={:.3} mxfp8_speedup_vs_bf16={:.4}x \
                 tensorwide_rel_l2={tensorwide_rel_l2:.8} mxfp8_rel_l2={mxfp8_rel_l2:.8} \
                 tensorwide_cosine={tensorwide_cosine:.8} mxfp8_cosine={mxfp8_cosine:.8} \
                 tensorwide_max_abs={tensorwide_max_abs:.6} mxfp8_max_abs={mxfp8_max_abs:.6}",
                tensorwide_quant.mean_us,
                tensorwide_gemm.mean_us,
                tensorwide_e2e.reference.mean_us,
                tensorwide_e2e.candidate.mean_us,
                tensorwide_e2e.candidate.p95_us,
                tensorwide_e2e.speedup_mean,
                mxfp8_quant.mean_us,
                mxfp8_gemm.mean_us,
                mxfp8_e2e.reference.mean_us,
                mxfp8_e2e.candidate.mean_us,
                mxfp8_e2e.candidate.p95_us,
                mxfp8_e2e.speedup_mean,
            );
        }
    }
    Ok(())
