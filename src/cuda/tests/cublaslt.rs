...
fn linear_bf16_layout_correctness() -> Result<()> {
    ...
    let weight_host = [
        // output 0
        1.0, 0.0, 0.0, // output 1
        0.0, 1.0, 0.0, // output 2
...
fn linear_bf16_reuses_cached_plan() -> Result<()> {
    ...
    let x = runtime.upload(&vec![bf16::from_f32(1.0); M * K], Shape::new([M, K]))?;

    let weight = runtime.upload(&vec![bf16::from_f32(0.5); N * K], Shape::new([N, K]))?;
