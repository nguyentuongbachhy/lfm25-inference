...
impl KernelSet for RmsNormKernels {
    ...
    fn from_module(module: Arc<CudaModule>) -> Result<Self> {
        ...
        let residual_rms_norm_bf16 = KernelLaunch::new_with_multiple(fused_function, MAX_BLOCK_SIZE, WARP_SIZE)?;
        ensure!(rms_norm_bf16.policy().block_size() >= WARP_SIZE, "RMSNorm requires at least one warp");
        ensure!(residual_rms_norm_bf16.policy().block_size() >= WARP_SIZE, "fused residual RMSNorm requires at least one warp");
        Ok(Self { rms_norm_bf16, residual_rms_norm_bf16 })
    }
}

impl RmsNormKernels {
    pub(crate) unsafe fn launch_bf16(&self, stream: &CudaStream, launch: RmsNormLaunch<'_>) -> Result<()> {
        let RmsNormLaunch { x, weight, out, rows, hidden_size, eps } = launch;
        ...
        args.arg(x).arg(weight).arg(out).arg(&rows).arg(&hidden_size).arg(&eps);
        unsafe { args.launch(config)?; }
        Ok(())
    }

    pub(crate) unsafe fn launch_residual_bf16(&self, stream: &CudaStream, launch: ResidualRmsNormLaunch<'_>) -> Result<()> {
        let ResidualRmsNormLaunch { residual, update, weight, residual_out, normalized_out, rows, hidden_size, eps } = launch;
        ensure!(rows > 0, "fused residual RMSNorm requires at least one row");
        ensure!(hidden_size > 0, "fused residual RMSNorm hidden size must be > 0");
        ...
        args.arg(residual).arg(update).arg(weight).arg(residual_out).arg(normalized_out).arg(&rows).arg(&hidden_size).arg(&eps);
        unsafe { args.launch(config)?; }
        Ok(())
    }
}
