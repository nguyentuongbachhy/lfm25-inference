...
impl RopeKernels {
    pub(crate) unsafe fn launch_qk_bf16_inplace(
        ...
        head_dim: usize,
            ...
            "RoPE head_dim={head_dim} exceeds \
             kernel maximum {MAX_HEAD_DIM}",
        );
        ...
        let half_dim = head_dim / 2;
            ...
            "RoPE inv_freq storage too small: \
             required={half_dim}, actual={}",
            inv_freq.len(),
        );

        ensure!(
            position_ids.len() >= num_tokens,
            "RoPE position_ids storage too small: \
             required={num_tokens}, actual={}",
            position_ids.len(),
        );
        ...
        let key_required = num_tokens
            ...
            "RoPE query storage too small: \
             required={query_required}, actual={}",
            query.len(),
        );

        ensure!(
            key.len() >= key_required,
            "RoPE key storage too small: \
             required={key_required}, actual={}",
            key.len(),
        );
