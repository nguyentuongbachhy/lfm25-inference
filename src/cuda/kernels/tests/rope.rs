    ...
    for (token, &position_id) in position_ids.iter().enumerate() {
        ...
        for head in 0..num_heads {
            ...
            for pair in 0..half_dim {
                ...
                /*
                 * CPU oracle keeps coefficients FP32,
                 * matching our optimized CUDA kernel.
                 */
...
fn rope_qk_bf16_correctness() -> Result<()> {
    ...
    for (num_tokens, num_q_heads, num_kv_heads, head_dim) in [
        ...
        // Main LFM2.5 shape.
        (4, 32, 8, 64),
        // Exercise max shared-cache range.
        (1, 2, 1, 512),
    ] {
...
fn bench_rope_qk_bf16() -> Result<()> {
    ...
    for num_tokens in [1, 4, 16, 64, 256, 1024, 4096] {
         ...
         *     inv_freq
         *     position_ids
         *     shared-memory coefficient traffic
         *
         * because Q/K traffic is the dominant,
         * easy-to-compare logical metric.
         */
