...
fn expected_embedding(
    ...
    for (token_idx, &token_id) in token_ids.iter().enumerate() {
        ...
        out[dst_start..dst_start + hidden_size]
...
fn run_embedding_case(
    ...
    unsafe {
        runtime.kernels().embedding().launch_bf16(
            runtime.stream(),
            EmbeddingLaunch {
                token_ids: token_ids_gpu.storage(),
                weight: weight_gpu.storage(),
                out: out.storage_mut(),
    ...
    assert_eq_bf16(&readback(runtime, &out)?, &expected);
    Ok(())
}
...
fn bench_embedding_bf16() -> Result<()> {
    ...
    for num_tokens in [1, 4, 16, 64, 256, 1024, 4096] {
        ...
        println!(
            "tokens={num_tokens:>5} | mean={:>8.3} us | p50={:>8.3} us | p95={:>8.3} us | min={:>8.3} us | {:>8.2} GB/s | {:>12.0} tok/s",
            stats.mean_us,
            stats.p50_us,
            stats.p95_us,
            stats.min_us,
