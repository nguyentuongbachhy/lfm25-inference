...
const MODULE_NAME: &str = "attention_prefill_fast_exp";
const PTX: &str = include_str!(concat!(env!("OUT_DIR"), "/attention_prefill_fast_exp.ptx"));
const THREADS: u32 = 256;
const Q_HEADS: usize = 32;
const KV_HEADS: usize = 8;
const HEAD_DIM: usize = 64;
const QUERY_TILE: usize = 2;
const TOKEN_COUNTS: &[usize] = &[512, 2048, 8192];
const ATOL: f64 = 0.035;
const RTOL: f64 = 0.025;

struct FastExpPrefillKernel {
    function: CudaFunction,
}

impl FastExpPrefillKernel {
...
unsafe fn launch_fast_exp(
    runtime: &CudaRuntime,
    kernel: &FastExpPrefillKernel,
    ...
    let blocks = query_tiles
        ...
        .context("fast-exp prefill grid size overflow")?;
    let grid_x = u32::try_from(blocks).context("fast-exp prefill grid exceeds u32")?;
...
#[test]
#[ignore = "GPU benchmark: Q2 contiguous prefill with CUDA fast exponential"]
fn bench_prefill_attention_fast_exp_bf16() -> Result<()> {
    let runtime = CudaRuntime::new(0)?;
    let fast = FastExpPrefillKernel::load(&runtime)?;

    for &num_tokens in TOKEN_COUNTS {
        let q_elements = num_tokens
            .checked_mul(Q_HEADS * HEAD_DIM)
            .context("fast-exp query element count overflow")?;
        let kv_elements = num_tokens
            .checked_mul(KV_HEADS * HEAD_DIM)
            .context("fast-exp KV element count overflow")?;
        ...
        let metrics = numerical_metrics(&reference_host, &candidate_host);
            ...
            "fast-exp prefill produced non-finite output at N={num_tokens}"
        );
        ensure!(
            metrics.within_tolerance,
            "fast-exp prefill exceeds decode fast-exp tolerance at N={num_tokens}: {metrics:?}"
        );
        ...
        println!(
            "prefill_fast_exp N={} precise_mean_us={:.3} fast_mean_us={:.3} speedup_mean={:.4}x precise_p50_us={:.3} fast_p50_us={:.3} speedup_p50={:.4}x precise_p95_us={:.3} fast_p95_us={:.3} speedup_p95={:.4}x speedup_min={:.4}x speedup_max={:.4}x max_abs={:.8} nrmse={:.8} cosine={:.8} non_finite={} within_tolerance={}",
            num_tokens,
