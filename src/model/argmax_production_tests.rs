...
fn bench_production_atomic_argmax_abba() -> Result<()> {
    ...
    for &(batch, context) in BENCH_SHAPES {
        ...
        // Balanced AB/BA across whole decode passes. Each pass starts from the
        // same deterministic prefill state and receives the same forced token
        // history. Only the final greedy argmax implementation differs.
        ...
        println!(
            ...
            top1_agreement,
            "atomic argmax production ABBA final top1 mismatch at B={batch} C={context}"
        );
    }
