...
const BENCH_MEASURED_STEPS: usize = 24;
const CACHE_DECODE_HEADROOM: usize = 64;

// These are the production-eligible shapes for which the target RTX 5060 ABBA
// measurements showed stable mean/p95 gains. B>16 is deliberately excluded
// because the production dispatcher falls back to the legacy implementation.
...
fn prefill_cache(
    ...
    let physical_pages = batch
        ...
        .context("atomic argmax benchmark page count overflow")?;
    let maximum_batch_tokens = batch
        .checked_mul(context)
        .context("atomic argmax benchmark prefill token count overflow")?;
...
fn run_decode_pass(
    ...
    set_atomic_argmax_test_override(Some(atomic_enabled));
    let result = (|| {
        ...
        for step in 0..BENCH_WARMUP_STEPS {
            ...
            let sampled = model
                ...
                .context("decode executor rejected atomic argmax warmup input")?;
        ...
        for measured in 0..BENCH_MEASURED_STEPS {
            ...
            let sampled = model
                ...
                .context("decode executor rejected atomic argmax benchmark input")?;
    ...
    // Always restore normal environment-controlled dispatch, including errors.
...
#[test]
#[ignore = "real checkpoint full-model atomic argmax ABBA benchmark"]
fn bench_production_atomic_argmax_abba() -> Result<()> {
    let runtime = CudaRuntime::new(0)?;
    let model = load_model(&runtime)?;

    for &(batch, context) in BENCH_SHAPES {
        let mut legacy_samples = Vec::with_capacity(BENCH_MEASURED_STEPS * 2);
        let mut atomic_samples = Vec::with_capacity(BENCH_MEASURED_STEPS * 2);
        let mut reference_trace: Option<Vec<Vec<u32>>> = None;
        let mut top1_agreement = true;

        // Balanced ABBA across complete decode passes. Every pass starts from
        // the same deterministic prefill and receives the same forced token
        // history. The entire warmup+measured sampled trace must be identical.
        for atomic_enabled in [false, true, true, false] {
            let pass = run_decode_pass(&runtime, &model, batch, context, atomic_enabled)?;
            ...
            if atomic_enabled {
                atomic_samples.extend(pass.samples_ms);
            } else {
                legacy_samples.extend(pass.samples_ms);
            }
        }

        let legacy_mean = mean(&legacy_samples);
        let atomic_mean = mean(&atomic_samples);
        let legacy_p95 = percentile(&legacy_samples, 0.95);
        let atomic_p95 = percentile(&atomic_samples, 0.95);
        println!(
            "atomic_argmax_production B={} C={} legacy_mean_ms={:.6} atomic_mean_ms={:.6} mean_speedup={:.4}x saving_us={:.3} legacy_p95_ms={:.6} atomic_p95_ms={:.6} p95_speedup={:.4}x top1_agreement={}",
            batch,
            context,
            legacy_mean,
            atomic_mean,
            legacy_mean / atomic_mean,
            (legacy_mean - atomic_mean) * 1000.0,
            legacy_p95,
            atomic_p95,
            legacy_p95 / atomic_p95,
            top1_agreement,
        );
        ensure!(
            top1_agreement,
            "atomic argmax production ABBA sampled-trace mismatch at B={batch} C={context}"
        );
    }
